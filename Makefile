...
UBBDCONF_HEADER := compat.h

$(UBBDCONF_HEADER):
	@> $@
	@echo $(CHECK_BUILD) compat-tests/have_nla_parse_nested_deprecated.c
	@if $(CHECK_BUILD) compat-tests/have_nla_parse_nested_deprecated.c > /dev/null 2>&1; then echo "#define HAVE_NLA_PARSE_NESTED_DEPRECATED 1"; else echo "/*#undefined HAVE_NLA_PARSE_NESTED_DEPRECATED*/"; fi >> $@
	@echo $(CHECK_BUILD) compat-tests/have_genl_small_ops.c
	@if $(CHECK_BUILD) compat-tests/have_genl_small_ops.c > /dev/null 2>&1; then echo "#define HAVE_GENL_SMALL_OPS 1"; else echo "/*#undefined HAVE_GENL_SMALL_OPS*/"; fi >> $@
	@echo $(CHECK_BUILD) compat-tests/have_genl_policy.c
	@if $(CHECK_BUILD) compat-tests/have_genl_policy.c > /dev/null 2>&1; then echo "#define HAVE_GENL_POLICY 1"; else echo "/*#undefined HAVE_GENL_POLICY*/"; fi >> $@
	@echo $(CHECK_BUILD) compat-tests/have_genl_validate.c
	@if $(CHECK_BUILD) compat-tests/have_genl_validate.c > /dev/null 2>&1; then echo "#define HAVE_GENL_VALIDATE 1"; else echo "/*#undefined HAVE_GENL_VALIDATE*/"; fi >> $@
	@echo $(CHECK_BUILD) compat-tests/have_alloc_disk.c
	@if $(CHECK_BUILD) compat-tests/have_alloc_disk.c > /dev/null 2>&1; then echo "#define HAVE_ALLOC_DISK 1"; else echo "/*#undefined HAVE_ALLOC_DISK*/"; fi >> $@
	@echo $(CHECK_BUILD) compat-tests/have_EXT_DEVT.c
	@if $(CHECK_BUILD) compat-tests/have_EXT_DEVT.c > /dev/null 2>&1; then echo "#define HAVE_EXT_DEVT 1"; else echo "/*#undefined HAVE_EXT_DEVT*/"; fi >> $@
	@echo $(CHECK_BUILD) compat-tests/have_flag_discard.c
	@if $(CHECK_BUILD) compat-tests/have_flag_discard.c > /dev/null 2>&1; then echo "#define HAVE_FLAG_DISCARD 1"; else echo "/*#undefined HAVE_FLAG_DISCARD*/"; fi >> $@
	@>> $@
...
EXTRA_CFLAGS += -I$(UBBD_KMODS_DIR)/ubbd-headers/ -DCOMMIT_REV="\"$(COMMIT_REV)\""
